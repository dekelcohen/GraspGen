...
def parse_args():
    ...
    parser.add_argument(
        "--data_dir",
        type=str,
        default=r"D:\NLP\Robotics\GraspModels\GraspGen\data\open_door_pybullet",
...
def load_and_unproject_pybullet_data(data_dir, target_seg_id):
    ...
    # 2. Extract valid pixels based on segmentation mask
    v, u = np.where(seg_mask == target_seg_id)
    if len(v) == 0:
        raise ValueError(f"No pixels found for target object ID {target_seg_id}")
    
    image_height, image_width = depth_array.shape
    
    # Extract depth and colors for masked pixels
    z_raw = depth_array[v, u]
    pc_colors = rgb_img[v, u, :3] # N x 3 (ensure we drop alpha if RGBA)
    
    # 3. Vectorized NDC Unprojection (matching new_3d_proj from robot.py)
    # Map Pixel X -> NDC X [-1.0, 1.0]
    ndc_x = (2.0 * u / image_width) - 1.0
    
    # Map Pixel Y -> NDC Y[1.0, -1.0]
    ndc_y = 1.0 - (2.0 * v / image_height)
    
    # Map Depth Buffer[0.0, 1.0] -> NDC Z [-1.0, 1.0]
    z_ndc = 2.0 * z_raw - 1.0
    
    # Create the Clip Space Vectors[x, y, z, 1.0] -> Shape: (4, N)
    clip_pos = np.stack([ndc_x, ndc_y, z_ndc, np.ones_like(ndc_x)], axis=0)
    
    # VP = Projection @ View
    VP = projection_matrix @ view_matrix
    inv_VP = np.linalg.inv(VP)
    
    # Apply Inverse VP: Clip Space -> World Space
    world_hom = inv_VP @ clip_pos
    
    # Perspective Divide
    pc_world_uncentered = (world_hom[:3, :] / world_hom[3, :]).T # Shape: N x 3
...
if __name__ == "__main__":
    ...
    pc_filtered_torch, pc_removed_torch, pc_color_filtered_torch, pc_color_removed_torch = point_cloud_outlier_removal_with_color(
        torch.from_numpy(pc_uncentered), torch.from_numpy(pc_color)
    )
